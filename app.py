...
def solve_northwest(origins, destinations, supply, demand):
    ...
    while i < len(origins) and j < len(destinations):
        ...
        if supply_copy[i] == 0:
            ...
            j += 1
    return {origins[i]: {destinations[j]: alloc[i][j] for j in range(len(destinations))} for i in range(len(origins))}
...
def solve_mincost(origins, destinations, supply, demand, costs):
    ...
    cells = sorted([(o, d, costs[o][d]) for o in origins for d in destinations], key=lambda x: x[2])
    for o, d, _ in cells:
...
def solve_vogel(origins, destinations, supply, demand, costs):
    ...
    while len(done_rows) < len(origins) and len(done_cols) < len(destinations):
        row_penalties = {}
        for o in origins:
            if o in done_rows: continue
            vals = sorted(costs[o][d] for d in destinations if d not in done_cols)
            if len(vals) >= 2: row_penalties[o] = vals[1] - vals[0]
            elif len(vals) == 1: row_penalties[o] = vals[0]
        col_penalties = {}
        for d in destinations:
            if d in done_cols: continue
            vals = sorted(costs[o][d] for o in origins if o not in done_rows)
            if len(vals) >= 2: col_penalties[d] = vals[1] - vals[0]
            elif len(vals) == 1: col_penalties[d] = vals[0]
        ...
        if max_row_pen >= max_col_pen and row_penalties:
            ...
            d = min((d for d in destinations if d not in done_cols), key=lambda d: costs[o][d])
        else:
            d = max(col_penalties, key=lambda k: col_penalties[k])
            o = min((o for o in origins if o not in done_rows), key=lambda o: costs[o][d])
        qty = min(supply_copy[o], demand_copy[d])
        allocation[o][d] += qty
        supply_copy[o] -= qty
        demand_copy[d] -= qty
        if supply_copy[o] == 0: done_rows.add(o)
        if demand_copy[d] == 0: done_cols.add(d)
    return allocation


def calc_total_cost(origins, destinations, allocation, costs):
    return sum(allocation[o][d] * costs[o][d] for o in origins for d in destinations)
...
def solve():
    ...
    costs = {o: {d: float(data["costs"][o][d]) for d in destinations} for o in origins}
    ...
    try:
        if method == "solver":
            allocation, total, status = solve_pulp(origins, destinations, supply, demand, costs)
            explanation = "Solución óptima encontrada con programación lineal (CBC solver)."
        elif method == "northwest":
            allocation = solve_northwest(origins, destinations, supply, demand)
            ...
            allocation = solve_mincost(origins, destinations, supply, demand, costs)
            total = calc_total_cost(origins, destinations, allocation, costs)
            explanation = "Solución inicial greedy. Se asigna primero a las rutas de menor costo."
        elif method == "vogel":
            allocation = solve_vogel(origins, destinations, supply, demand, costs)
            total = calc_total_cost(origins, destinations, allocation, costs)
        ...
        routes = [
            {"from": o, "to": d, "qty": allocation[o][d],
             "unit_cost": costs[o][d], "subtotal": allocation[o][d] * costs[o][d]}
            for o in origins for d in destinations if allocation[o][d] > 0
        ]
